 ...
 * Uses venv Python if available, otherwise falls back to python3.
 */
function getPythonPath(): string {
  // Try to get Python from the environment manager (uses venv)
  const venvPython = pythonEnvManager.getPythonPath();
  if (venvPython && existsSync(venvPython)) {
    return venvPython;
  }
  // Fall back to python3 (standard on Linux/macOS)
  return 'python3';
}
